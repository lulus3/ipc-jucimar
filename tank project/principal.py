...
while loop_game:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            loop_game = False
        if event.type == pygame.KEYDOWN:
            keystrokes.keys_down(event, list_of_tank)
    ...
    if not animation:
        ...
        for tank in list_of_tank:
            if (tank.movement and tank.spin_right) or (tank.movement and tank.spin_left):
                tank.movement = False
                tank.spin_right = False
                tank.spin_left = False

            # analyze recharge condition
            if counter - tank.time_to_recharge > recharge_time:
                tank.recharge = False
            ...
            for obstacle in list_of_obstacle:
                ...
                if tank.tank_rect.colliderect(obstacle_rect):
                    pos = collision_tank_or_ball(
